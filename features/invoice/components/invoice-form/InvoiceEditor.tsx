...
function InvoiceEditor() {
  ...
  return (
    <>
      ...
      <div className="lg:hidden w-full h-full p-5  bg-gray-50">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="bg-white h-full border border-gray-200 shadow-sm py-2 px-3 rounded-lg w-full grid grid-cols-2">
            <TabsTrigger
              value="invoice-details"
              className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-blue-600 data-[state=active]:to-cyan-500 data-[state=active]:text-white data-[state=active]:shadow-lg text-gray-700 font-medium py-2.5 px-4 rounded-md transition-all duration-200 data-[state=active]:transform data-[state=active]:scale-[1.02]"
            >
              Invoice Details
            </TabsTrigger>
            <TabsTrigger
          ...
          <TabsContent value="invoice-details" className="mt-6 ">
            <ScrollArea className="h-[calc(100vh-200px)] pb-20 pr-5">
              {viewMode === "invoice-details" && <InvoiceDetails />}
              {viewMode === "layout" && <InvoiceTemplate />}
              {viewMode === "theme" && <InvoiceTheme />}
            </ScrollArea>
          </TabsContent>
          <TabsContent value="invoice-preview" className="mt-6">
        ...
        <div className="fixed block [800px]:hidden bottom-0 left-0 w-full border-t border-gray-200 shadow-xl backdrop-blur-sm bg-white/95">
          <div className="flex items-center justify-between px-5 py-4 max-w-screen-xl mx-auto">
            <Button
              variant="outline"
              size="default"
              className="border-2 border-blue-600 text-blue-600 hover:bg-gradient-to-r hover:from-blue-600 hover:to-cyan-500 hover:text-white font-medium py-2.5 px-5 rounded-lg shadow-lg hover:shadow-xl transition-all duration-200 transform hover:scale-105 bg-white"
              onClick={() => {
                setActiveTab(
