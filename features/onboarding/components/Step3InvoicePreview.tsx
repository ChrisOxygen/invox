...
export function Step3InvoicePreview() {
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <div className="pl-6 pr-5 py-5 flex flex-col gap-4">
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex flex-col gap-1.5">
              {profilePreview.logoUrl ? (
                ...
                <div className="flex items-center justify-center shrink-0 w-10 h-10 rounded-md bg-(--blue-100)">
                  <span className="[font-family:var(--font-display)] font-bold text-[18px] text-(--blue-600)">
                    {initials}
                  </span>
                </div>
              )}
              <span className="[font-family:var(--font-display)] font-bold text-[15px] text-(--ink-900)">
                {displayName}
              </span>
            </div>
            ...
            <div className="flex flex-col items-end gap-1">
              <span className="[font-family:var(--font-display)] font-extrabold text-[18px] text-(--blue-600) tracking-[0.08em]">
                INVOICE
              </span>
          ...
          {/* Bill to */}
          <div className="flex flex-col gap-1">
            <span className="[font-family:var(--font-display)] font-semibold uppercase text-[10px] text-(--ink-300) tracking-[0.08em]">
              Bill To
            </span>
            <span className="[font-family:var(--font-body)] text-[14px] text-(--ink-700)">
              Acme Corp
            </span>
          </div>
          ...
          <div className="flex flex-col rounded-md overflow-hidden border border-(--border-default)">
            ...
            >
              {(['Description', 'Qty', 'Unit Price', 'Total'] as const).map((h) => (
                <span
                  key={h}
                  className="[font-family:var(--font-display)] font-semibold uppercase text-[10px] text-(--ink-300) tracking-[0.08em]"
            ...
            >
              <span className="[font-family:var(--font-body)] text-[13px] text-(--ink-900)">Design Services</span>
            ...
            <div
              ...
              style={{ gridTemplateColumns: '1fr 40px 90px 90px', padding: '7px 10px' }}
            >
              <span className="[font-family:var(--font-body)] text-[13px] text-(--ink-900)">Brand Strategy</span>
          ...
          <div className="flex flex-col items-end gap-1.5">
            <div className="flex items-center gap-8">
              <span className="[font-family:var(--font-body)] text-[13px] text-(--ink-400)">Subtotal</span>
            ...
            <div className="flex items-center gap-8 pt-1.5 border-t border-(--border-default)">
              <span className="[font-family:var(--font-display)] font-bold text-[16px] text-(--ink-900)">Total Due</span>
              <span className="font-mono font-bold text-[18px] text-(--blue-600)">₦450,000.00</span>
            </div>
          </div>

          {/* Footer */}
          <div className="pt-3 [font-family:var(--font-body)] italic text-[12px] text-(--ink-300) border-t border-(--border-default)">
      ...
      {/* CTA */}
      <Button className="w-full h-11 [font-family:var(--font-display)] font-semibold" onClick={nextStep}>
        Continue
      </Button>
    </div>
  )
